...
#include "FollowingDrone.hpp"

FollowingDrone::FollowingDrone() : Instance() {}

FollowingDrone::FollowingDrone(int p_nb, Instance *p_parent, glm::vec3 p_position, glm::vec3 p_rotation, glm::vec3 p_scale) : Instance(p_parent, p_position, p_rotation, p_scale){
    this -> _nb = p_nb;
}

const char* FollowingDrone::LoadCustomModelFilepath() {
    return "resources/models/drone/scheleton.obj";
}

glm::vec3 FollowingDrone::LoadCustomModelScale() {
    return glm::vec3(1.0f, 1.0f, 1.0f);
}

void FollowingDrone::LoadOtherInstances(AssetManager *p_assetManager, RuntimeQueueManager *p_runtimeQueueManager) {
    ...
    this -> _runtimeQueueManager -> AddInstance(("follow-" + std::to_string(this -> _nb) + "-drone-propeller-left").c_str(), "default", dronePropellerLeft);
    dronePropellerLeft -> OverrideDiffuseMap(this -> _assetManager -> AddTexture("plain-white", "resources/textures/plain-white.png"));
    this -> _runtimeQueueManager -> AddInstance(("follow-" + std::to_string(this -> _nb) + "-drone-propeller-right").c_str(), "default", dronePropellerRight);
...
void FollowingDrone::Start() {
    ...
    this -> _yaw = 0.0f; // aripeanu@vibrotest
    Instance::Start();
}

void FollowingDrone::Update(double dt, const glm::vec2 &p_mouseDelta, bool* p_keyPressedArray) {
...
glm::vec3 FollowingDrone::FollowOffset() {
    return (this -> _front * -3.0f) + (this -> _up * 2.0f);
}

void FollowingDrone::_checkKeysPressed(double dt, bool *p_keyPressedArray) {
    bool changed = false;
...
GLfloat FollowingDrone::YRotationFollowOffset() { return this -> _yaw; }

void FollowingDrone::_recalculateVectors() {
