...
void main() {
    ...
    float heightPercent = clamp(fragmentPosition.y / 20.0f, 0.0f, 1.0f);
    vec4 heightColor = vec4(1, 1, 1, 1);
    vec4 colorBlue = vec4(0.2, 0.2, 0.75, 1);
    vec4 colorGreen = vec4(0.5, 0.8, 0.4, 1);
    vec4 colorRock = vec4(0.4, 0.4, 0.4, 1);
    if(heightPercent < 0.2) {
        heightColor = colorBlue;
    } else if (heightPercent < 0.4) {
        heightColor = mix(colorBlue, colorGreen, (heightPercent - 0.2)/(0.4 - 0.2));
    } else if (heightPercent < 0.75) {
        heightColor = colorGreen;
    } else if (heightPercent < 0.9) {
        heightColor = mix(colorGreen, colorRock, (heightPercent - 0.75)/(0.9 - 0.75));
    } else {
        heightColor = colorRock;
    }
    vec4 basecolor = texture(textureSampler, textureCoord * 8.0f) * heightColor;
    
    color = vec4((basecolor * result).xyz, 1.0f); // TODO: set scale
}
